...
exports.sendBulkEmail = exports.sendMail = void 0;
...
const sendColdEmail = (email, name) => {
    return new Promise((resolve, reject) => {
        const mailOptions = {
            from: "creatorships02@gmail.com",
            to: email,
            subject: "Connect with Top Creators to Boost Your Brand on Creatorships",
            text: getText(name),
            html: getHtml(name),
        };
        // Send email
...
const sendBulkEmail = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        for (let i = 0; i < emails.length; i++) {
            ...
            if (email && name) {
                try {
                    let response = yield sendColdEmail(email, name);
                    if (response.isError) {
                        count++;
                    }
                }
            ...
            yield delay(5000); // Delay of 5 seconds
        }
        if (count === emails.length) {
            ...
            res.status(200).json({
                isError: false,
                message: `Email sent successfully to ${emails.length - count} recipients`,
            });
        }
    }
    catch (error) {
